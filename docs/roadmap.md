...
3. Clone do repositório: `git clone https://github.com/contadoriadf/dashboard`
...
### Objetivo
Criar um segundo repositório GitHub público (`contadoriadf/relatorios`) para dashboards de uso interno com dados D-0 (Oracle em tempo real), atualizado várias vezes ao dia. O repositório existente (`dashboard`) não é alterado.

### Arquitetura

```
Repo 1 (existente)                    Repo 2 (novo)
contadoriadf/dashboard                contadoriadf/relatorios
──────────────────────                ──────────────────────
...
1. **Criar repo** `contadoriadf/relatorios` no GitHub (público, GitHub Pages na `main`)
...
3. **Instalar segundo runner** na mesma máquina:
   ...
   .\config.cmd --url https://github.com/contadoriadf/relatorios --token <TOKEN>
